      ...
      {!canManage ? <p className="mt-4 text-sm">Ask your workspace owner or admin to configure Zernio.</p> : <>
        ...
        {!data?.configured ? <form className="mt-5 space-y-3" onSubmit={e => { e.preventDefault(); void act({ method: 'POST', body: { apiKey } }); }}>
          ...
          <form className="space-y-3" onSubmit={e => { e.preventDefault(); void act({ method: 'PUT', body: { profileId } }); }}>
            <label htmlFor="zernio-profile" className="block text-sm font-medium">Zernio profile</label>
            <select id="zernio-profile" value={profileId} onChange={e => setProfileId(e.target.value)} required className="w-full rounded-lg border border-zernio-border bg-white px-3 py-2 text-sm"><option value="">Select a profile</option>{data.profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
            {!data.profiles.length && <p className="text-sm">Create a profile in Zernio, then refresh this page.</p>}
